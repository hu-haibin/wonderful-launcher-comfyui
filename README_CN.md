...
[**下载最新安装包**](https://github.com/hu-haibin/wonderful-launcher-comfyui/releases/latest) · [**2.0.17 更新说明**](https://github.com/hu-haibin/wonderful-launcher-comfyui/releases/tag/v2.0.17) · [**2.0 系列归档**](#20-系列发布归档) · [**反馈问题**](https://github.com/hu-haibin/wonderful-launcher-comfyui/issues)
...
发布于 2026 年 5 月 25 日。[打开 2.0.17 GitHub Release](https://github.com/hu-haibin/wonderful-launcher-comfyui/releases/tag/v2.0.17)。

这个版本重点解决在线图片长任务的恢复问题：即使任务超过桌面端会话时间，重新打开后也能找回结果。

| 方向 | 变化 |
|------|------|
| **图片任务恢复** | 4K 等长任务会进入服务端任务表，桌面端重启后可以从任务列表恢复已完成结果，而不是因为超时丢图。 |
| **云端结果镜像** | 完成的图片可以通过 R2 回写并从云端任务列表展示，减少对上游临时 URL 的依赖。 |
| **Worker 兜底** | Cloudflare Worker 轮询兜底可以在桌面端关闭后继续推进任务，并在失败时按任务安全退款。 |
| **本地历史安全** | Image 右侧缩略图改成分页展示，历史变多时不再自动删除用户花积分生成的本地图片文件。 |
| **Image 真机验证** | 已覆盖 1K 文生图、图生图参考图、四图生成、4K 重启恢复、深浅色/语言切换和关闭保护提示。 |
| **发版可靠性** | 公开安装包继续使用 self-contained WinUI 打包，并通过 web 测试、完整 Release 测试、发布构建和隔离启动 smoke。 |
