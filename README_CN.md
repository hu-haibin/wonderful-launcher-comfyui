...
- **普通用户推荐**：[Wonderful Launcher 2.0.46 Setup 安装包](https://github.com/hu-haibin/wonderful-launcher-comfyui/releases/tag/v2.0.46-repair-setup)
- **更新说明**：[Wonderful Launcher 2.0.46](https://github.com/hu-haibin/wonderful-launcher-comfyui/releases/tag/v2.0.46)
- **公开稳定回退版本**：[ModelFinder 2.0.31](https://github.com/hu-haibin/wonderful-launcher-comfyui/releases/tag/v2.0.31)

打开安装包 release，下载 `WonderfulLauncher-Setup-v2.0.46.exe`。
...
发布于 2026 年 6 月 24 日。[查看完整 GitHub Release](https://github.com/hu-haibin/wonderful-launcher-comfyui/releases/tag/v2.0.46)。

这次更新重点收紧 Agent 启动修复在 ComfyUI 成功重启后的收口逻辑，并让 Avalonia 通知在切换语言或打开工作区后保持可读。

- **Agent 启动修复更容易正确收口**：一次重启失败不会再挡住后续成功重试进入验证；即使日志里出现 custom nodes，启动失败修复也会优先归类为运行时修复；已经验证成功的会话不会继续挂着旧失败分类。
- **ComfyUI ready 判断更快**：后端验证会被 ComfyUI ready 输出或进程退出主动唤醒，不再只等下一次定时轮询；嵌入式工作区验证也会根据 WebView / 运行状态变化立即继续检查，同时保留轮询兜底。
- **Avalonia 通知 toast 会随语言刷新**：依赖崩溃/启动异常通知会保留资源 key，切换语言时刷新当前 toast，并在打开嵌入式 ComfyUI 工作区后移动到右上角，避免遮住工作区。
- **更新恢复指向当前安装包**：更新代理和修复下载路径已经准备好使用 2.0.46 安装器，并继续保持自包含 Windows 安装包形态。
...
1. 打开 [安装包下载 release](https://github.com/hu-haibin/wonderful-launcher-comfyui/releases/tag/v2.0.46-repair-setup)。
2. 下载 `WonderfulLauncher-Setup-v2.0.46.exe`。
...
- [Wonderful Launcher 2.0.46 更新说明](https://github.com/hu-haibin/wonderful-launcher-comfyui/releases/tag/v2.0.46)
- [Wonderful Launcher 2.0.46 Setup 安装包](https://github.com/hu-haibin/wonderful-launcher-comfyui/releases/tag/v2.0.46-repair-setup)
