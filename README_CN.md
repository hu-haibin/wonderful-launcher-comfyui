...
大多数共享的 ComfyUI 工作流并不是下载后就能一键运行。它们常常依赖特定模型、自定义节点、Python 包、PyTorch 版本、目录位置、启动参数，以及环境损坏后的修复步骤。

Wonderful Launcher 把这些维护动作放到本地 ComfyUI 运行环境旁边：

| 你需要 | Wonderful Launcher 做什么 |
|--------|---------------------------|
| 接管已有 ComfyUI | 导入 ComfyUI Desktop 或便携版 ComfyUI。 |
| 从零开始 | 下载并部署一个干净的 ComfyUI 包。 |
| ComfyUI 启动失败 | 查看启动日志，并执行你批准的依赖修复任务。 |
| 工作流缺模型 | 匹配缺失模型名，并下载或放置到预期目录。 |
| 工作流缺节点 | 安装自定义节点和依赖，不用手动翻文件夹。 |
| 多个包共用模型 | 在不同 ComfyUI 包之间共享模型目录。 |
| 生成和复用图片 | 把图片生成、参考图、历史记录和 Photoshop 发送放在同一个工作区。 |
