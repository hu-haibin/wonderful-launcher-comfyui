...
发布于 2026 年 6 月 17 日。[查看完整 GitHub Release](https://github.com/hu-haibin/ModelFinder-Releases/releases/tag/v2.0.29)。

这次更新继续沿着 Avalonia 主线收口，把几个真正影响日常工作流的细节补齐了。

- **应用更新进度终于可见**：Avalonia 现在会把更新下载/应用进度直接显示在标题栏下方，不再像静默更新时那样看起来没反应。
- **工作流模板可以一键接到工作区**：点击模板卡片后，会自动打开或启动内嵌 ComfyUI 工作区，并把对应工作流直接导入进去。
- **模板预览可以动起来**：官方工作流模板里使用 GIF / WebP / APNG 的预览资源，现在在 Avalonia 里也能播放，视频类模板更容易提前判断。
- **部署页浅色主题更干净**：ComfyUI 部署页里磁盘和 GPU 的选中态改成了更轻的浅色样式，不会再显得发闷发重。
