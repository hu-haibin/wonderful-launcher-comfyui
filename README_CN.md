...
[**下载安装包**](https://github.com/hu-haibin/wonderful-launcher-comfyui/releases/tag/v2.0.47-repair-setup) · [**更新说明**](https://github.com/hu-haibin/wonderful-launcher-comfyui/releases/tag/v2.0.47) · [**官网**](https://wonderfullauncher.com/) · [**文档**](https://wonderfullauncher.com/docs) · [**反馈问题**](https://github.com/hu-haibin/wonderful-launcher-comfyui/issues)
...
- **普通用户推荐**：[Wonderful Launcher 2.0.47 Setup 安装包](https://github.com/hu-haibin/wonderful-launcher-comfyui/releases/tag/v2.0.47-repair-setup)
- **更新说明**：[Wonderful Launcher 2.0.47](https://github.com/hu-haibin/wonderful-launcher-comfyui/releases/tag/v2.0.47)
- **公开稳定回退版本**：[ModelFinder 2.0.31](https://github.com/hu-haibin/wonderful-launcher-comfyui/releases/tag/v2.0.31)

打开安装包 release，下载 `WonderfulLauncher-Setup-v2.0.47.exe`。
...
发布于 2026 年 6 月 25 日。[查看完整 GitHub Release](https://github.com/hu-haibin/wonderful-launcher-comfyui/releases/tag/v2.0.47)。

这次更新重点收紧 Agent 对 PyTorch / CUDA 运行时问题的判断，减少错误工具调用，并让不完整的 ComfyUI 目录走更清晰的恢复路径。

- **PyTorch 运行时保护更严格**：Agent 会把 `torch-directml` 也视为受保护运行时后端，torch 系列包统一走启动器原生运行时修复路径，避免通过通用 pip 操作把运行时降级或装散。
- **CPU 索引误判更清楚**：当 NVIDIA 场景需要 CUDA，但运行时解析只拿到 CPU PyTorch 索引时，启动器会返回结构化的不可重试信号，减少 Agent 在同一条错误安装路径上循环。
- **兼容新版 NVIDIA 驱动输出**：CUDA 探测现在支持新版 `nvidia-smi` 里的 `CUDA UMD Version`，降低 CUDA 机器被误判为 CPU-only 的概率。
- **损坏的 ComfyUI 根目录走设置/重部署路径**：缺少 `requirements.txt`、`main.py`、`folder_paths.py` 或 `comfy/` 这类核心文件时，会被归类为状态/部署问题，而不是泛化成权限或 pip 失败。
...
1. 打开 [安装包下载 release](https://github.com/hu-haibin/wonderful-launcher-comfyui/releases/tag/v2.0.47-repair-setup)。
2. 下载 `WonderfulLauncher-Setup-v2.0.47.exe`。
...
详细版本变化请看 GitHub Release notes：

- [最新版本](https://github.com/hu-haibin/wonderful-launcher-comfyui/releases/latest)
- [Wonderful Launcher 2.0.47 更新说明](https://github.com/hu-haibin/wonderful-launcher-comfyui/releases/tag/v2.0.47)
- [Wonderful Launcher 2.0.47 Setup 安装包](https://github.com/hu-haibin/wonderful-launcher-comfyui/releases/tag/v2.0.47-repair-setup)
