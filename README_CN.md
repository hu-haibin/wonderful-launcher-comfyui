...
- **普通用户推荐**：[Wonderful Launcher 2.0.35](https://github.com/hu-haibin/wonderful-launcher-comfyui/releases/tag/v2.0.35)
...
发布于 2026 年 6 月 19 日。[查看完整 GitHub Release](https://github.com/hu-haibin/wonderful-launcher-comfyui/releases/tag/v2.0.35)。

这个版本重点收口 Avalonia 主线的更新链路、更新缓存和品牌露出，同时保留旧 `2.0.x` ModelFinder 安装的原地升级路径。

- **自动更新下载更干净**：更新安装包现在进入受管理的本地更新缓存，不再长期堆在用户的 Downloads 目录里。
- **更新清理更安全**：安装器交接成功后会清理已完成的安装包或解压文件，同时保护仍在等待应用的更新。
- **公开品牌更一致**：Avalonia 启动、更新相关界面和发布资产继续统一到 `Wonderful Launcher`。
- **升级链路已复测**：重新验证了 Windows 上从 `2.0.32` 原地升级到 `2.0.35` 的安装器链路。
- **发版诊断更稳**：围绕 `WonderfulLauncher-Setup-v2.0.35.exe` 补强了更新缓存和安装器交接覆盖。
