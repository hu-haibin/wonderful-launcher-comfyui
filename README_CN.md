...
[**下载 2.1.11**](https://github.com/hu-haibin/wonderful-launcher-comfyui/releases/tag/v2.1.11) · [**更新说明**](https://github.com/hu-haibin/wonderful-launcher-comfyui/releases/tag/v2.1.11) · [**官网**](https://wonderfullauncher.com/) · [**文档**](https://wonderfullauncher.com/docs) · [**反馈问题**](https://github.com/hu-haibin/wonderful-launcher-comfyui/issues)
...
这个版本主要把 ComfyUI 启动和缺失模型处理变得更顺：少打扰、少绕路，尽量在用户真正需要下载模型的 WebView 工作区里完成。

- **启动 ComfyUI 更干净**：启动前不再因为 Torch 探测超时弹出误导性的修复框。真正的 ComfyUI 进程依赖失败仍会记录日志，并在错误明确时进入依赖修复。
- **缺失模型回到工作区处理**：移除了工具箱里独立的模型查找入口。WebView 通知条会直接打开当前工作流的缺失模型列表，用正在运行的 ComfyUI 工作流上下文来匹配。
- **模型结果更少歧义**：列表聚焦工作流里的模型名、目标模型目录、匹配状态和下载动作；底部操作也改为“复制模型名称”和“下载全部”。
- **共享模型目录更简洁**：ComfyUI 设置里的模型共享改成紧凑的 A → B 布局，并能显示当前包是否正在使用全局共享模型路径。
- **图片生成小体验优化**：图片生成对话里的 prompt 文本可以更自然地选择和复制。
- **发版验证**：self-contained v2.1.11 包已通过完整 Release 测试和发布根目录启动 smoke。
...
1. 打开 [2.1.11 安装包 release](https://github.com/hu-haibin/wonderful-launcher-comfyui/releases/tag/v2.1.11)。
...
- [Wonderful Launcher 2.1.11 更新说明](https://github.com/hu-haibin/wonderful-launcher-comfyui/releases/tag/v2.1.11)
- [Wonderful Launcher 2.1.11 Setup 安装包](https://github.com/hu-haibin/wonderful-launcher-comfyui/releases/tag/v2.1.11)
