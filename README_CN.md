...
[**下载 2.1.15**](https://github.com/hu-haibin/wonderful-launcher-comfyui/releases/tag/v2.1.15) · [**官网**](https://wonderfullauncher.com/) · [**文档**](https://wonderfullauncher.com/docs) · [**反馈问题**](https://github.com/hu-haibin/wonderful-launcher-comfyui/issues)
...
- **普通用户推荐**：[Wonderful Launcher 2.1.15 Setup 安装包](https://github.com/hu-haibin/wonderful-launcher-comfyui/releases/tag/v2.1.15)
- **安装包文件**：`WonderfulLauncher-Setup-v2.1.15.exe`
...
发布于 2026 年 8 月 11 日。[查看完整 GitHub Release](https://github.com/hu-haibin/wonderful-launcher-comfyui/releases/tag/v2.1.15)。

- **桌面导航继续收口**：Canvas Lab 不再出现在公开导航中，普通图片工作区仍是支持的生成入口。
- **本地模型拖放更有用**：拖入的模型无法匹配当前工作流时，现在可以直接选择当前 ComfyUI models 目录内的目标文件夹，不会停在无路可走的提示上。
- **运行时操作更清晰**：ComfyUI 的启动和停止状态使用不同的视觉样式，更容易看清下一步操作。
- **更新验证更稳妥**：发布和安装器 smoke 现在会确认发布根目录、隔离配置和安装器交接都确实属于正在验证的候选版本。

本次打包前已通过工作区、本地化、更新器和更新代理的定向测试；自包含安装包与发布根目录隔离 smoke 会作为本次发版流程的一部分验证。
...
1. 打开 [最新版安装包 release](https://github.com/hu-haibin/wonderful-launcher-comfyui/releases/tag/v2.1.15)。
2. 下载 `WonderfulLauncher-Setup-v2.1.15.exe`。
