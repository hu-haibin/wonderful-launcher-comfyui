...
这个版本重点打磨 Avalonia 图片工作区，让长时间生图时不再把下一轮创作卡死。

- **支持并行追加生图轮次**：前一轮还在排队或生成时，你可以继续编辑并发送下一轮图片请求。
- **参考图 / 历史图交互更一致**：左侧 `input` 参考图历史现在更接近右侧历史栏，支持查看、拖回、复用和上下文操作。
- **拖拽回填更稳**：恢复更大的参考图区命中范围，把图片拖回 composer 的体验更接近旧版本。
- **生成占位更清楚**：加载占位卡复用了工作流模板的描边流光动画，状态跟随卡片本身，不再用漂浮的全局提示。
- **修复回车发送和再生成回归**：Enter 发送与右键菜单里的“再生成一张”都补回了最近的回归问题。
...
| 版本 | 日期 | 摘要 |
|------|------|------|
| **2.0.31** | 2026 年 6 月 19 日 | Avalonia 图片工作区版本，重点是多轮并行生图、更清楚的占位卡、拖拽回归修复，以及回车/再生成修复。 |
