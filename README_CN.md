...
- **普通用户推荐**：[Wonderful Launcher 2.0.37](https://github.com/hu-haibin/wonderful-launcher-comfyui/releases/tag/v2.0.37)
...
发布于 2026 年 6 月 20 日。[查看完整 GitHub Release](https://github.com/hu-haibin/wonderful-launcher-comfyui/releases/tag/v2.0.37)。

这次更新主要收口 Agent Access 上线后的桌面 Agent 和工作区体验。

- **Agent 路由交给服务端统一管理**：桌面端不再携带过期的 codex/MiniMax provider 偏好；当前 DeepSeek 优先和兜底策略由云端 relay 统一决定。
- **Agent 状态文案更准确**：状态卡不再显示旧 provider 名称，避免排障时误判。
- **Agent Access 次数显示对齐**：桌面端和网站端现在对付费 Agent Access 总次数、剩余次数的展示保持一致。
- **工作区修复入口更稳定**：缺节点/缺模型通知条保持主要修复动作可见，减少工作流切换时忽隐忽现的干扰。
