...
[**下载最新安装包**](https://github.com/hu-haibin/wonderful-launcher-comfyui/releases/latest) · [**2.0.23 更新说明**](https://github.com/hu-haibin/wonderful-launcher-comfyui/releases/tag/v2.0.23) · [**2.0 系列归档**](#20-系列发布归档) · [**反馈问题**](https://github.com/hu-haibin/wonderful-launcher-comfyui/issues)
...
发布于 2026 年 6 月 2 日。[打开 2.0.23 GitHub Release](https://github.com/hu-haibin/wonderful-launcher-comfyui/releases/tag/v2.0.23)。

这个版本是在 2.0.22 自定义节点下载修复之后，对 WinUI 桌面体验做的一轮打磨。它让内置 ComfyUI WebView 的图片拖入更接近浏览器行为，让启动器浮窗稳定盖在工作区上方，并收紧 Image、Agent 和缺失节点恢复交互。

| 方向 | 变化 |
|------|------|
| **ComfyUI WebView 图片拖入** | 把图片拖到 Load Image 节点时会替换该节点里的图片；拖到空白画布时可以创建新的 Load Image 节点，更接近浏览器里的 ComfyUI 行为。 |
| **下载浮窗行为** | 下载浮窗会稳定显示在内置 WebView 上方；已完成下载行优先提供“打开模型所在目录”，不再把低价值的单条删除放在主要位置。 |
| **Agent 标题栏体验** | Agent 入口移动到标题栏，标题栏按钮和主题切换更一致，Agent 历史浮窗布局收紧，行内容和删除操作不再互相挤压。 |
| **Image 页操作** | 调整图片结果操作，提示词 hover 按钮不再盖住提示词；动态文案会随语言切换刷新；新版和旧版 Image 的动作菜单共用本地化标签。 |
| **缺失节点恢复** | 优化缺失节点安装后的验证逻辑，减少插件已在重启后正确注册却仍提示“仍有节点缺失”的误判。 |
| **发版验证** | Release 编译 0 warning。重点 UX/恢复测试通过 164 项；Release 全量测试通过 2,196 项；发布根目录 smoke 在隔离 profile 下拉起 `ModelFinder.App.exe`，日志里没有 error/fatal/unhandled。 |
| **发布资产** | 公开下载形态仍是 setup 安装包加 `SHA256SUMS.txt`，没有新增 portable 包。 |

<p align="center">
  <img src="assets/screenshots/feature-model-finder.png" alt="工作流缺失模型查找" width="32%" />
  <img src="assets/screenshots/2.0.23-download-flyout.png" alt="显示已完成模型任务的下载浮窗" width="32%" />
...
GitHub Releases 页面只保留适合普通用户下载的版本。2.0.23 发布后，2.0.0 到 2.0.22 的中间 release 页面已下架，避免用户误装被后续替代的版本，尤其是 2.0.7/2.0.8 的打包回归版本。
...
<p align="center">
  ...
  <img src="assets/screenshots/2.0.23-download-flyout.png" alt="ModelFinder 下载浮窗" width="32%" />
  <img src="assets/screenshots/2.0.23-image-workspace.png" alt="ModelFinder 图片工作区" width="32%" />
</p>
