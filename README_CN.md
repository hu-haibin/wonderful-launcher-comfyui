...
### `Wonderful Launcher` 是桌面产品名；`ModelFinder` 只保留在不能轻易改动的内部兼容标识中。
...
- **普通用户推荐**：[Wonderful Launcher 2.0.38](https://github.com/hu-haibin/wonderful-launcher-comfyui/releases/tag/v2.0.38)
- **公开稳定回退版本**：[ModelFinder 2.0.31](https://github.com/hu-haibin/wonderful-launcher-comfyui/releases/tag/v2.0.31)

产品和桌面应用统一叫 `Wonderful Launcher`。内部目录、进程名、AppId、日志或迁移标记仍可能保留 `ModelFinder`，以免破坏已有安装和升级链。
...
Wonderful Launcher 的目标，是把这些事情收回到一个桌面工作流里，让你可以：
...
发布于 2026 年 6 月 21 日。[查看完整 GitHub Release](https://github.com/hu-haibin/wonderful-launcher-comfyui/releases/tag/v2.0.38)。

这次更新集中修复 Avalonia 部署体验、ComfyUI Desktop 接管后的内核切换，以及自动更新路径兼容。

- **Avalonia 部署设置更清楚**：ComfyUI 部署设置里，磁盘和运行包选中项在深色/浅色主题下都保持可读。
- **任务终端复制链接修复**：重复的“复制当前链接/复制官方链接”合并为一个可靠复制动作。
- **接管 ComfyUI Desktop 后切内核更安全**：切换前会识别未跟踪的桌面迁移文件，并支持先 stash，避免 checkout 时突然失败。
- **更新源迁移兼容**：自动更新主仓切到 `wonderful-launcher-comfyui`，同时保留旧 `ModelFinder-Releases` 兜底。
- **Agent 结果语义治理**：云端 Agent 质量记录对 `verified_fixed` 的误判处理更一致。

> [!NOTE]
> 部分进程级诊断或系统标识里，仍可能暂时看到 `ModelFinder` 或 `mf`；公开安装器和安装表面使用 `Wonderful Launcher`。
...
Wonderful Launcher 是面向用户的产品名。部分内部标识会继续保留 `ModelFinder` 或 `mf`，用于兼容已有安装目录、更新链、日志和 Windows 身份。
