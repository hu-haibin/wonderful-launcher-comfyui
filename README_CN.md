...
[**下载最新安装包**](https://github.com/hu-haibin/wonderful-launcher-comfyui/releases/latest) · [**2.0.21 更新说明**](https://github.com/hu-haibin/wonderful-launcher-comfyui/releases/tag/v2.0.21) · [**2.0 系列归档**](#20-系列发布归档) · [**反馈问题**](https://github.com/hu-haibin/wonderful-launcher-comfyui/issues)
...
发布于 2026 年 5 月 31 日。[打开 2.0.21 GitHub Release](https://github.com/hu-haibin/wonderful-launcher-comfyui/releases/tag/v2.0.21)。

这个版本是 Banana Pro 和参考图工作流上线后的 Image 页稳定性热修。它移除了 release smoke 中复现的 WinUI 原生崩溃路径，继续保持公开安装包为 self-contained，并重新验证真实在线 Image 链路。

| 方向 | 变化 |
|------|------|
| **Image 崩溃热修** | 移除历史缩略图上的长 prompt tooltip。编辑、复制、用作参考图等操作仍保留在查看器和右键菜单里，避免历史 rail 重建时悬挂 tooltip 触发 WinUI 原生崩溃。 |
| **回归保护** | 增加 release-readiness 源码测试，防止这条高风险缩略图 tooltip 路径被悄悄加回来。 |
| **Image 真机验证** | 使用登录账号完成 GPT Image 2 / 1K / 1 张参考图的 img2img，任务完成、保存到本地历史，并按预期扣除 3 积分。 |
| **发布根启动验证** | 验证发布根目录的 watchdog 入口能在隔离 profile 下拉起 `ModelFinder.App.exe`，并正常关闭且没有 WER 崩溃。 |
| **安装包覆盖升级验证** | 验证临时目录内 `2.0.20 -> 2.0.21` 覆盖安装成功，然后恢复真实 HKCU 卸载注册表和桌面快捷方式。 |
| **发版可靠性** | 公开安装包继续使用 self-contained WinUI 打包，release 资产只包含安装包和校验文件。 |
...
| 2.0.19 | 2026 年 5 月 27 日 | 已被 2.0.20 替代 | 增加带登录态的 Image 结果兜底下载、任务归属校验、历史恢复元数据和设置入口交接修复。 |
| 2.0.20 | 2026 年 5 月 28 日 | 已被 2.0.21 替代 | 将更新下载路由到 Wonderful Launcher 代理/CDN，增加更新源控制、更新取消能力，并改善 Image 参考图预上传。 |
