...
[**下载最新安装包**](https://github.com/hu-haibin/wonderful-launcher-comfyui/releases/latest) · [**2.0.22 更新说明**](https://github.com/hu-haibin/wonderful-launcher-comfyui/releases/tag/v2.0.22) · [**2.0 系列归档**](#20-系列发布归档) · [**反馈问题**](https://github.com/hu-haibin/wonderful-launcher-comfyui/issues)
...
发布于 2026 年 6 月 1 日。[打开 2.0.22 GitHub Release](https://github.com/hu-haibin/wonderful-launcher-comfyui/releases/tag/v2.0.22)。

这个版本是工作流下载可靠性更新。它修复了由工作流解析出来、必须安装到 ComfyUI `custom_nodes` 而不是普通 `models` 目录的模型下载路径，同时保持公开安装包仍为 self-contained，release 资产仍只包含安装包和校验文件。

| 方向 | 变化 |
|------|------|
| **自定义节点模型目标** | 云端模型目录可以解析安全的 `custom_nodes/...` 保存路径，并在写入前拦截路径穿越和绝对路径。 |
| **缺失模型下载** | WebView 批量下载规划现在可以把匹配到的工作流资源安装到 `models` 或 `custom_nodes`，让自定义节点资产落到节点实际读取的位置。 |
| **解析器覆盖** | 增加自定义节点目标、`models/` 前缀归一化、危险自定义节点路径拦截的回归测试。 |
| **测试环境加固** | Avalonia 混合 UI/非 UI 测试会更可靠地泵送已初始化 dispatcher 的待处理任务。 |
| **发版验证** | Release 全量测试通过，共 2,193 项；发布根目录 smoke 在隔离 profile 下拉起 `ModelFinder.App.exe`，日志里没有 error/fatal/unhandled。 |
...
<p align="center">
  ...
  <img src="assets/screenshots/2.0.22-download-flyout.png" alt="显示已完成模型任务的下载浮窗" width="32%" />
...
GitHub Releases 页面只保留适合普通用户下载的版本。2.0.22 发布后，2.0.0 到 2.0.21 的中间 release 页面已下架，避免用户误装被后续替代的版本，尤其是 2.0.7/2.0.8 的打包回归版本。
...
<p align="center">
  ...
  <img src="assets/screenshots/2.0.22-download-flyout.png" alt="ModelFinder 下载浮窗" width="32%" />
  <img src="assets/screenshots/2.0.22-image-workspace.png" alt="ModelFinder 图片工作区" width="32%" />
</p>
