...
[**下载最新安装包**](https://github.com/hu-haibin/wonderful-launcher-comfyui/releases/latest) · [**2.0.24 更新说明**](https://github.com/hu-haibin/wonderful-launcher-comfyui/releases/tag/v2.0.24) · [**2.0 系列归档**](#20-系列发布归档) · [**反馈问题**](https://github.com/hu-haibin/wonderful-launcher-comfyui/issues)
...
发布于 2026 年 6 月 2 日。[打开 2.0.24 GitHub Release](https://github.com/hu-haibin/wonderful-launcher-comfyui/releases/tag/v2.0.24)。

这个 hotfix 替换了 2.0.23 的 WebView 宿主路线。经过 Windows 10 和 Windows 11 真机验证后，WinUI 默认回到交互更稳定的 XAML WebView，并通过带坐标的拖拽桥接恢复 ComfyUI 工作流和图片拖入，同时下架被替代的 2.0.x 版本，避免用户下载到有问题的构建。

| 方向 | 变化 |
|------|------|
| **WinUI ComfyUI WebView** | 默认工作区宿主回到 XAML WebView2，保证 Windows 11 上节点点击、画布拖动和平移缩放稳定。原生 HWND 宿主只保留为 `MODELFINDER_WINUI_WEBVIEW_HOST=native` 诊断路径。 |
| **工作区文件拖入** | 本地工作流和图片拖入会带着原始投放坐标桥接到 ComfyUI。图片拖到 `Load Image` 节点会替换该节点；拖到空白画布会创建新的图片节点。 |
| **真机验证** | Windows 10 和 Windows 11 均验证了工作流拖入、图片拖入、`Load Image` 替换图片，以及 ComfyUI 工作区正常交互。 |
| **发版验证** | Release 全量测试通过 2,196 项；发布根目录 smoke 在隔离 profile 下拉起 `ModelFinder.App.exe`，日志里没有 error/fatal/unhandled。 |
...
GitHub Releases 页面只保留适合普通用户下载的版本。2.0.24 发布后，2.0.0 到 2.0.23 的中间 release 页面已下架，避免用户误装被后续替代的版本、2.0.7/2.0.8 的打包回归版本，或 2.0.23 的 WinUI WebView 宿主回归版本。
